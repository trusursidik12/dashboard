...
class B_bulanan_m extends CI_Model {
  ...
  public function get_aqmdata_month($idstasiun,$month,$parameter){
    ...
    $this->db->where('waktu LIKE date_format(waktu, "%'.$month.'%")');
    $query = $this->db->get();
    return $query->result_array();
  }

  public function get_max_aqmispu_month($idstasiun,$month){
    $sql = $this->db->query('select id_stasiun, waktu, pm10, so2, co, o3, no2, @highest_val:=greatest(pm10,so2,co,o3,no2) AS maxvalueparams, CASE @highest_val WHEN waktu THEN "WAKTU" WHEN pm10 THEN "PM10" WHEN so2 THEN "SO2" WHEN co THEN "CO" WHEN o3 THEN "O3" WHEN no2 THEN "NO2" END AS maxnameparams from aqm_ispu WHERE id_stasiun = "'.$idstasiun.'" AND waktu LIKE date_format(waktu, "%'.$month.'%")');
