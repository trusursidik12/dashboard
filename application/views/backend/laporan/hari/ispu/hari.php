      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title"><?= empty($title_header) ? 'Laporan ISPU Harian' : ''.$title_header ?></h3>
          </div>
          <!-- /.card-header -->
          <div class="card-body table-responsive">
            <div class="row">
              <div class="col-md-10 col-md-offset-2">
                <form class="form-inline"  method="post" action="<?= site_url('laporan/ispu/hari') ?>">
            ...
            <div id="cetak">
                              ...
                              <td>
                                <?php if(!empty($maxispu)) : ?>
                                  <?php foreach($maxispu as $berlaku) : ?>
                                    <?php $datetime = new DateTime(date('Y-m-d', strtotime($berlaku['waktu']))) ?>
                                    <?php $datetime->modify('+1 day'); ?>
                                    <?= 'Berlaku : '.date('d/m/Y', strtotime($berlaku['waktu'])).' 15:00'.' - '.$datetime->format('d/m/Y').' 15:00' ?>
                                  <?php endforeach ?>
                                <?php else : ?>
                                  <?= "Bulan" ?>
                                <?php endif ?>
              ...
              <table class="table table-bordered table-striped text-center" width="100%">
                <tr>
                  <td width="200" style="text-align: left;">Kualitas Udara</td> <td width="10">:</td> <td>
                    <?php if(!empty($maxispu)) : ?>
                      <?php foreach($maxispu as $maxispuvalue) : ?>
                        <?php if($maxispuvalue['maxvalueparams'] >= 0 && $maxispuvalue['maxvalueparams'] <= 50) : ?>
                          <?= "BAIK" ?>
                        <?php elseif($maxispuvalue['maxvalueparams'] > 50 && $maxispuvalue['maxvalueparams'] <= 100) : ?>
                          <?= "SEDANG" ?>
                        <?php elseif($maxispuvalue['maxvalueparams'] > 100 && $maxispuvalue['maxvalueparams'] <= 199) : ?>
                          <?= "TIDAK SEHAT" ?>
                        <?php elseif($maxispuvalue['maxvalueparams'] > 199 && $maxispuvalue['maxvalueparams'] <= 299) : ?>
                          <?= "SANGAT TIDAK SEHAT" ?>
                        <?php elseif($maxispuvalue['maxvalueparams'] > 299) : ?>
                          <?= "BERBAHAYA" ?>
                        <?php endif ?>
                      <?php endforeach ?>
                    <?php else : ?>
                ...
                <tr>
                  <td width="200" style="text-align: left;">Komponen</td>
                  <td width="10">:</td>
                  <td>
                    <?php if(!empty($maxispu)) : ?>
                      <?php foreach($maxispu as $maxispuparams) : ?>
                      <?= $maxispuparams['maxnameparams'] ?>
                      <?php endforeach ?>
                      <?php else : ?>
                        <?= '-' ?>
                    <?php endif ?>
